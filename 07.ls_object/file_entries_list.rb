...
class FileEntriesList
  ...
  def run_ls_short(width)
    max_file_name_count = @entries.map { |f| f.stats[:name].size }.max
    col_count = width / (max_file_name_count + 1)
    row_count = col_count.zero? ? @entries.count : (@entries.count.to_f / col_count).ceil
    transposed_file_entries = safe_transpose(@entries.each_slice(row_count).to_a)
